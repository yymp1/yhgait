# 当前交付索引

## 当前收口优先看

- `reports/gui_copy_refresh.md`
- `reports/private_gallery_setup.md`
- `reports/private_gallery_schema.md`
- `reports/private_gallery_validation.md`
- `reports/private_gallery_scope.md`
- `reports/private_gallery_recognition_flow.md`
- `reports/effect_analysis_summary.md`
- `reports/private_gallery_next_steps.md`
- `reports/real_video_demo_setup.md`
- `reports/real_video_demo_validation.md`
- `reports/real_video_demo_scope.md`
- `reports/real_video_demo_input_requirements.md`
- `reports/real_video_demo_failure_modes.md`
- `reports/real_video_demo_next_steps.md`
- `reports/gait_demo_analysis.md`
- `reports/gait_demo_casebook.md`
- `reports/gait_demo_scope.md`
- `reports/gait_demo_setup.md`
- `reports/gait_demo_validation.md`
- `reports/formal_conservative_checkpoint_eval_comparison.md`
- `reports/training_decision_review.md`
- `reports/formal_conservative_final_summary.md`
- `reports/formal_conservative_eval.md`
- `reports/formal_conservative_snapshot.md`
- `reports/formal_conservative_training.md`
- `reports/formal_conservative_assessment.md`
- `reports/opengait_training_readiness.md`
- `README.md`

## 私有步态库 Demo

- `reports/gui_copy_refresh.md`
- `reports/private_gallery_setup.md`
- `reports/private_gallery_schema.md`
- `reports/private_gallery_validation.md`
- `reports/private_gallery_scope.md`
- `reports/private_gallery_recognition_flow.md`
- `reports/effect_analysis_summary.md`
- `reports/private_gallery_next_steps.md`

## 最先看

- `reports/linux_third_round.md`
- `reports/linux_midrun_validation.md`
- `reports/linux_longrun_validation.md`
- `reports/night_run_assessment.md`
- `reports/training_stability_assessment.md`
- `reports/linux_baseline_small.md`
- `reports/checkpoint_resume_validation.md`
- `reports/training_recipe_recommendation.md`
- `reports/real_data_validation.md`
- `reports/casia_b_download.md`
- `reports/opengait_training_readiness.md`
- `reports/morning_handoff.md`

## 真实视频 Demo

- `reports/real_video_demo_setup.md`
- `reports/real_video_demo_validation.md`
- `reports/real_video_demo_scope.md`
- `reports/real_video_demo_input_requirements.md`
- `reports/real_video_demo_failure_modes.md`
- `reports/real_video_demo_next_steps.md`

## 正式保守训练与评估收口

- `reports/formal_conservative_training.md`
- `reports/formal_conservative_assessment.md`
- `reports/formal_conservative_eval.md`
- `reports/formal_conservative_snapshot.md`
- `reports/formal_conservative_final_summary.md`

## 中程验证与稳定性

- `reports/linux_midrun_validation.md`
- `reports/training_stability_assessment.md`
- `reports/linux_third_round/baseline_midrun_50.md`
- `reports/linux_third_round/baseline_midrun_100.md`

## 长跑验证与夜跑评估

- `reports/linux_longrun_validation.md`
- `reports/night_run_assessment.md`
- `reports/linux_third_round/baseline_longrun_300.md`
- `reports/linux_third_round/baseline_longrun_500.md`

## 训练前检查

- `reports/pretreatment_preflight.md`
- `reports/casiab_pkl_check.md`
- `reports/opengait_smoke_test.md`

## 本机 CPU/gloo 兼容探针

- `reports/cpu_probe_smoke.md`
- `reports/cpu_probe_short_run.md`

## 环境与依赖

- `scripts/setup_opengait_pretreatment_env.py`
- `reports/opengait_training_env_freeze.txt`

## 补丁与坏帧

- `docs/opengait_local_patch.md`
- `patches/pretreatment_skip_bad_frames.patch`
- `reports/opengait_local_patch_summary.md`
- `reports/casia_b_bad_frames.tsv`

## 迁移与下一步

- `README.md`
- `reports/morning_handoff.md`
